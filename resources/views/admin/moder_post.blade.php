    ...
    <div class="bg-gray-100 min-h-screen py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-8 flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-900">Модерация публикаций</h1>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $post->created_at->format('d.m.Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex gap-2">
                                        <form action="{{ route('moder.posts.reject', $post) }}" method="POST">
                                            @csrf @method('PUT')
                                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg transition text-xs">
                                                Отклонить
                                            </button>
                                        </form>
                                        <form action="{{ route('moder.posts.approve', $post) }}" method="POST">
                                            @csrf @method('PUT')
                                            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-lg transition text-xs">
                                                Одобрить
                                            </button>
                                        </form>
                                    </div>
